...
export default function Shuffler() {
  ...
  useEffect(() => {
    if (totalScore == targetPoints){
      ...
      setFinish(true)
      let dat = locSecStore.getItem("e2_player")
      locSecStore.setItem("e2_player", {...dat, e2_sfcd: true})
    }
  }, [totalScore]);
    ...
    <div className="flex justify-center items-center w-screen h-screen">
      ...
      <div className="relative">
        
        {
          pos.map((e, i) => {
            return (
            <div key={e.id} id={e.id} className="absolute text-black flip" style={{transform: `translate(${e.x}, ${e.y})`, transition: 'transform 1s', width:`${boxW}rem`, height:`${boxH}rem`}} onClick={flipCard.bind(this, i)}>
              <div className={`h-full bg-white cursor-pointer flip-card ${e.flipped ? "flip-now": ""}`}>
                <div className="flip-card-inner">
                  <div className="flip-card-front bg-white">
                    <p>{e.id.substring(0,0)}</p>
                  </div>
                  <div className="flip-card-back border border-white" style={{backgroundColor: `#${e.color}`}}>
                    <h1 className="text-4xl flex justify-center items-center w-full h-full"> {e.backText}</h1>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="absolute top-0 left-0 mt-12 w-full text-center">
        <div className="border h-2 w-1/3 mx-auto">
          <div className="bg-blue-500 h-2" style={{width: `${Math.min(totalScore, targetPoints)/targetPoints*100}%`}}>&nbsp;</div>
        </div>
      </div>
    </div>
  );
