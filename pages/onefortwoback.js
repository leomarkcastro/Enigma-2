...
export default function Orbs() {
  ...
  const [cList, setCList] = useState([true,false,false,false,false,false,false,false,false,false,false])
  ...
  return (
    <div className="flex flex-col justify-center items-center w-screen h-screen">
      ...
      <div id="lamps" className="orbsCont mb-16">
       {
         lamps.map((e,i) => <label key={i} style={{transform: `translate(${e.x}px, ${e.y}px)`}}>
                              <input 
                                type="checkbox"
                                checked={cList[i]}
      ...
      <p className="text-sm text-center">There are {orbs} lamps placed in a circle. You may toggle any three of them that go in a row.</p>
      <p className="text-sm text-center">One lamp is switched on at the start. Can you turn on all of them?</p>
      </div>
